...
import frisket
from frisket.runtime.model_install import model_server_source
...
def run_probe(*, checkout_root: Path, workspace: Path) -> None:
    ...
    model_server_path = model_server_source()
    _assert(
        model_server_path.is_relative_to(package_path.parent),
        f"model server source is outside the installed package: {model_server_path}",
    )
