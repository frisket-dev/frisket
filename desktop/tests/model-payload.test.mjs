...
test('prewarm bundles only OCR and VAD without downloading transcription weights', async (t) => {
  ...
  assert.deepEqual(JSON.parse(result.stdout), { rapidocr: 'ready', vad: 'vad' });
  assert.deepEqual((await fs.readFile(trace, 'utf8')).trim().split(/\r?\n/), ['rapidocr', 'vad', 'vad']);
  const ref = spawnSync(python, ['-I', path.resolve('scripts/prewarm-models.py'), resources, '--whisper-ref'], {
    cwd: path.resolve('.'), encoding: 'utf8', env,
  });
  assert.equal(ref.status, 0, ref.stderr);
  assert.equal(ref.stdout.trim(), 'hf-snapshot:whisper@rev');
  assert.deepEqual((await fs.readFile(trace, 'utf8')).trim().split(/\r?\n/), ['rapidocr', 'vad', 'vad']);
});
