...
var FbTable = class _FbTable {
  ...
  table(index) {
    ...
    return new _FbTable(this.dv, off + this.dv.getInt32(off, true));
  }
  string(index) {
    const off = this.fieldOffset(index);
    if (off === 0) return null;
  ...
  tableVector(index) {
    ...
    for (let i = 0; i < length; i += 1) {
      ...
      out.push(new _FbTable(this.dv, elemOff + this.dv.getInt32(elemOff, true)));
    }
    return out;
  }
...
function parseSchemaField(field) {
...
function readValidity(dv, bodyStart, buffer, count) {
...
function decodeMapPointsArrow(buf) {
  ...
  const readMessage = () => {
    ...
    if (continuation !== 4294967295) throw decodeError("missing continuation marker");
  ...
  for (const field of fields) {
    ...
    attributes[field.name.slice("attr:".length)] = raw.map(
      (value) => value == null ? null : typeof value === "bigint" ? Number(value) : value
    );
  }
  return {
    count: total,
...
var NUMERIC_LOW = [222, 235, 247];
var NUMERIC_HIGH = [8, 81, 156];
...
function buildPointStyle({
  ...
  const colorVals = colorBy ? attrs[colorBy] : void 0;
  const sizeVals = sizeBy ? attrs[sizeBy] : void 0;
  ...
  if (colorVals && colorNumeric && colorMax >= colorMin && colorMin !== Infinity) {
    ...
    legend = {
      kind: "category",
      name: colorName,
      items: Array.from(catColor.entries()).slice(0, 10).map(([label, color]) => ({ label, color }))
    };
  }
  return {
    getFillColor,
...
var DEFAULT_BASEMAP_TILE_URL = "https://tile.openstreetmap.org/{z}/{x}/{y}.png";
var DEFAULT_BASEMAP_ATTRIBUTION = "\xA9 OpenStreetMap contributors";
function resolveBasemapConfig() {
  const runtime = typeof window !== "undefined" ? window.__FRISKET_MAP_CONFIG__ ?? {} : {};
  const apiKey = runtime.apiKey?.trim() ?? "";
  const tileUrl = (runtime.tileUrlTemplate?.trim() || DEFAULT_BASEMAP_TILE_URL).split("{key}").join(apiKey);
  return {
    tileUrl,
    attribution: runtime.attribution?.trim() || DEFAULT_BASEMAP_ATTRIBUTION
  };
}
function viewStateForPoints(positions, count) {
  if (count === 0) {
...
function isViewportUserInteraction(interactionState) {
...
function viewportBboxFromElement(deckgl, el, viewState) {
...
function MapToolbar({
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "map-view-toolbar" }, /* @__PURE__ */ React.createElement("span", { className: "map-view-title" }, columnName), /* @__PURE__ */ React.createElement("span", { className: "map-view-stats", "data-testid": "map-points-count" }, pointCount !== null ? `${pointCount} points` : loading ? "Loading\u2026" : ""), /* @__PURE__ */ React.createElement("label", { className: "map-view-control" }, "View", /* @__PURE__ */ React.createElement(
    "select",
    {
    ...
    /* @__PURE__ */ React.createElement("option", { value: "points" }, "Points"),
    /* @__PURE__ */ React.createElement("option", { value: "heatmap" }, "Heatmap")
  )), /* @__PURE__ */ React.createElement("label", { className: "map-view-control" }, "Color by", /* @__PURE__ */ React.createElement(
    "select",
    {
      className: "row-height-select",
    ...
    colorByColumns(columns, columnId).map((c) => /* @__PURE__ */ React.createElement("option", { key: String(c.id), value: String(c.id) }, c.name))
  )), /* @__PURE__ */ React.createElement("label", { className: "map-view-control" }, "Size by", /* @__PURE__ */ React.createElement(
    "select",
    {
      className: "row-height-select",
    ...
    /* @__PURE__ */ React.createElement("option", { value: "" }, "none"),
    sizeByColumns(columns, columnId).map((c) => /* @__PURE__ */ React.createElement("option", { key: String(c.id), value: String(c.id) }, c.name))
  )), onFilterToViewport && /* @__PURE__ */ React.createElement(
    "button",
    {
      type: "button",
      className: "mini-btn",
    ...
    "Filter to this area"
  ), transient && /* @__PURE__ */ React.createElement("span", { className: "map-view-transient", "data-testid": "map-transient" }, "updating\u2026"));
}
function MapLegend({ React, legend }) {
  if (!legend) return null;
  return /* @__PURE__ */ React.createElement("div", { className: "map-legend", "data-testid": "map-legend" }, /* @__PURE__ */ React.createElement("div", { className: "map-legend-title" }, legend.name), legend.kind === "numeric" ? /* @__PURE__ */ React.createElement("div", { className: "map-legend-numeric" }, /* @__PURE__ */ React.createElement(
    "span",
    {
  ...
  ), /* @__PURE__ */ React.createElement("span", { className: "map-legend-range" }, legend.min, " \u2013 ", legend.max)) : legend.items.map((it) => /* @__PURE__ */ React.createElement("div", { className: "map-legend-item", key: it.label }, /* @__PURE__ */ React.createElement(
    "span",
    {
...
function MapTooltip({
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("div", { className: "map-tooltip-coords" }, tooltip.lat.toFixed(5), ", ", tooltip.lon.toFixed(5)),
    tooltip.lines.map((l) => /* @__PURE__ */ React.createElement("div", { key: l.label }, /* @__PURE__ */ React.createElement("span", { className: "map-tooltip-label" }, l.label, ":"), " ", l.value))
  );
}
function mapControllerReducer(state, action) {
  switch (action.type) {
    case "set_color_by":
...
function MapView({ React, ctx }) {
  const { useCallback, useEffect, useMemo, useReducer, useRef, useState } = React;
  const [deckgl, setDeckgl] = useState(null);
  const [deckglError, setDeckglError] = useState(null);
  const [state, dispatch] = useReducer(mapControllerReducer, initialMapControllerState);
  const containerRef = useRef(null);
  const canvasRef = useRef(null);
  const deckRef = useRef(null);
  const bboxDebounce = useRef(null);
  const fittedGenerationRef = useRef(null);
  const stateRef = useRef(state);
  stateRef.current = state;
  const ctxRef = useRef(ctx);
  ctxRef.current = ctx;
  const columns = ctx?.sheet?.columns ?? [];
  const routedColumnId = typeof ctx?.projection?.params?.targetColumnId === "string" || typeof ctx?.projection?.params?.targetColumnId === "number" ? String(ctx.projection.params.targetColumnId) : null;
  const routedColumn = routedColumnId ? columns.find((c) => String(c.id) === routedColumnId && c.type === "geo_point") ?? null : null;
  ...
  const gridStateKey = ctx?.gridState ? JSON.stringify([ctx.gridState.filter, ctx.gridState.sort]) : "";
  useEffect(() => {
    let active = true;
    if (!ctx?.libs?.deckgl) {
    ...
    ctx.libs.deckgl.then((namespace) => {
  ...
  useEffect(() => {
    ...
    ctx.projection.fetchData({
      ...
      const fittedViewState = fittedGenerationRef.current !== decoded.generation ? viewStateForPoints(decoded.positions, decoded.count) : null;
  ...
  const { tileUrl: basemapTileUrl, attribution: basemapAttribution } = resolveBasemapConfig();
  ...
  useEffect(() => {
    if (!deckgl || !canvasRef.current || state.error) return void 0;
  ...
  const filterToViewport = useMemo(() => {
    if (!gridFilter || !columnId) return void 0;
    return () => {
  ...
  if (!ctx || ctx.schemaVersion !== "frisket.plugin_projection_view_context.v1") {
    return /* @__PURE__ */ React.createElement("div", { className: "map-view", "data-testid": "map-view" }, /* @__PURE__ */ React.createElement("div", { className: "map-view-error", "data-testid": "map-error" }, "map view mounted without its projection view context"));
  }
  if (!columnId) {
    return /* @__PURE__ */ React.createElement("div", { className: "map-view", "data-testid": "map-view" }, /* @__PURE__ */ React.createElement("div", { className: "map-view-empty", "data-testid": "map-empty" }, "No geo_point column in this sheet."));
  }
  const error = deckglError ?? state.error;
  return /* @__PURE__ */ React.createElement("div", { className: "map-view", "data-testid": "map-view" }, /* @__PURE__ */ React.createElement(
    MapToolbar,
    {
      React,
      columnId,
  ...
  ), /* @__PURE__ */ React.createElement("div", { className: "map-view-canvas", "data-testid": "map-canvas-container", ref: containerRef }, error ? /* @__PURE__ */ React.createElement("div", { className: "map-view-error", "data-testid": "map-error" }, error) : /* @__PURE__ */ React.createElement(
    "canvas",
    {
      ref: canvasRef,
      style: {
  ...
  ), state.viewMode === "points" && /* @__PURE__ */ React.createElement(MapTooltip, { React, tooltip }), !error && /* @__PURE__ */ React.createElement(MapLegend, { React, legend: pointStyle.legend }), !error && /* @__PURE__ */ React.createElement("div", { className: "map-view-attribution", "data-testid": "map-basemap-attribution" }, basemapAttribution), !error && state.result && state.result.count === 0 && !state.loading && /* @__PURE__ */ React.createElement("div", { className: "map-view-empty", "data-testid": "map-empty" }, "No mappable points in this column.")));
}
export {
  MapView,
  decodeMapPointsArrow
};
