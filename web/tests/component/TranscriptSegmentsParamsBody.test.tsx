...
import { TranscriptSegmentsParamsBody } from '../../src/components/action-panel/TranscriptSegmentsParamsBody';

afterEach(cleanup);

type Props = ComponentProps<typeof TranscriptSegmentsParamsBody>;
...
const sheet: SheetMeta = {
  ...
  columns: [
    { id: '1', name: 'transcript', type: 'timestamped_transcript' },
    { id: '2', name: 'alternate', type: 'timestamped_transcript' },
...
const request: Request = {
  ...
  sheet_name: 'Transcript segments', output_names: { transcript: 'Excerpt' },
};
const points: Params = {
  source: 'transcript',
...
function Harness({ initial = points, context = request, metadata = sheet }: {
  ...
  const Field: Props['Field'] = ({ name, testId }) => name === 'source'
    ? <select data-testid={testId} value={params.source}
        ...
        {metadata.columns.filter((c) => c.type === 'timestamped_transcript').map((c) => (
          <option key={c.id} value={c.name}>{c.name}</option>
        ))}
      </select>
    : <pre data-testid="saved-selection">{JSON.stringify(params.selection)}</pre>;
  return <>
    <TranscriptSegmentsParamsBody params={params} setParams={setParams}
...
describe('TranscriptSegmentsParamsBody', () => {
  it('preserves transcript range and point controls without owning names or Run', async () => {
    ...
    expect(screen.getByTestId('split-draft-preview')).toHaveTextContent('Boundaries include any transcript chunk they touch');
    expect(current()).toEqual({ source: 'transcript', selection: {
    ...
    expect(screen.getByTestId('split-draft-preview')).toHaveTextContent('whole chunk in both neighboring rows');
  ...
  it('column selection uses all-row scope without literal acknowledgement or retargeting missing input', async () => {
    ...
    expect(screen.getByTestId('transcript-segments-params')).toHaveTextContent('all 4 rows');
  ...
  it.each([
    ...
    render(<Harness initial={{ source: 'transcript', selection }} />);
